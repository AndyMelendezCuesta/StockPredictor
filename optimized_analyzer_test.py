...
def rollingWindow(seq, windowSize):
    it = iter(seq)
    #win is the iterative version of the given sequence (the 5 closing prices passed by historical prices)
    win = [it.next() for cnt in xrange(windowSize)] # First window
    yield win #array of arrays
    print "first yield: ", win
    ...
    for e in it: # Subsequent windows
        ...
        win[-1] = e #the last value from the win list = e (element from the sequence-the 5 closing prices passed by historical prices)
...
def getMovingAverage(values, windowSize):
    ...
    for w in rollingWindow(values, windowSize): #we iterate over the new ordered sequence-the 5 closing prices passed by historical prices accomodated in a different way (some elements were excluded. Line 49 and 50)
    ...
    return movingAverages #averages from the new ordered sequence-the 5 closing prices passed by historical prices accomodated in a different way (some elements were excluded. Line 49 and 50)
...
def getMinimums(values, windowSize):
    ...
    return minimums #minimum values from the new ordered sequence-the 5 closing prices passed by historical prices accomodated in a different way (some elements were excluded. Line 49 and 50)
...
def getMaximums(values, windowSize):
    ...
    return maximums #maximum values from the new ordered sequence-the 5 closing prices passed by historical prices accomodated in a different way (some elements were excluded. Line 49 and 50)
...
def getTrainingData(symbol, chosen_date):
    ...
    print "this is trainingData, Hey!: ", trainingData
    return trainingData
...
def given_data():
    ...
    predictions = []
    #expected
    expected.append(get_historical_prices('GOOG', '2014-04-04')[1]) #weekday (Friday)

    #predictions
    predictions.append(analyzeSymbol('GOOG', '2014-04-03')) #weekday (Thursday)
